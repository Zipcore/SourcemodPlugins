		...
		if(IsClientInGame(i) && IsClientConnected(i))
		{
			if(GetClientAimTarget(i, false) == -1)
			{
				...
				}else if(CheckOwner(i, iEnt))
				{
						...
						{
							Format(sTemp, sizeof(sTemp), "Url: %s", g_sUrlName[iEnt]);
						}else{
							if(StrEqual(sClassname, "cel_sound", false))
							{
								Format(sTemp, sizeof(sTemp), "Sound: %s", g_sSoundName[iEnt]);
							}else{
								if(StrEqual(sClassname, "cel_music", false))
								{
									Format(sTemp, sizeof(sTemp), "Song: %s", g_sSongName[iEnt]);
								}else{
									Format(sTemp, sizeof(sTemp), "Prop: %s", g_sPropName[iEnt]);
								}
							}
					}
				}else{
						if(StrEqual(sClassname, "cel_internet", false))
						{
							Format(sTemp, sizeof(sTemp), "Owner: %N\nUrl: %s", g_iOwner[iEnt], g_sUrlName[iEnt]);
						}else{
							if(StrEqual(sClassname, "cel_sound", false))
							{
								Format(sTemp, sizeof(sTemp), "Owner: %N\nSound: %s", g_iOwner[iEnt], g_sSoundName[iEnt]);
							}else{
								if(StrEqual(sClassname, "cel_music", false))
								{
									Format(sTemp, sizeof(sTemp), "Owner: %N\nSong: %s", g_iOwner[iEnt], g_sSongName[iEnt]);
								}else{
									Format(sTemp, sizeof(sTemp), "Owner: %N\nProp: %s", g_iOwner[iEnt], g_sPropName[iEnt]);
								}
							}
						}
				}
...
public Action:Command_Delete(iClient, iArgs)
{
	...
	if(g_fCommandTime[iClient] < GetGameTime() - 1)
	{
		...
		if(CheckOwner(iClient, iEnt))
		{
			...
			if(StrContains(sClassname, "cel_", false) != -1)
			{
...
public Action:Command_Owner(iClient, iArgs)
{
	...
	{
		Format(sTemp, sizeof(sTemp), "The player's name is {green}%N{default}.", iEnt);
	}else{
